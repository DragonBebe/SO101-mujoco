...
def evaluate(simulation):
    ...
    inputs = ('RGB + optical-depth' if simulation.cameras.depth_available else 'RGB only')
    result = {'task':simulation.task, 'seed':simulation.seed, 'success':bool(info.get('success')),
              'metrics':metrics, 'cameras':simulation.camera_report(),
              'policy_input':f'{inputs} + camera calibration + robot proprioception',
