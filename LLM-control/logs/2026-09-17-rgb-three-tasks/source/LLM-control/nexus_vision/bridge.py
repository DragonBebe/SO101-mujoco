...
def serve(args, session_factory=VisualSession):
    ...
    # Camera options are optional CLI extras: the older vision.py entry point
    # has no such flags and must keep its original positional call.
    extra={name:getattr(args,name) for name in ('camera_modality','environment_camera')
           if getattr(args,name,None) is not None}
    session=session_factory(args.record,args.task,args.seed,args.viewer,args.realtime,
                            args.camera_viewer,**extra)
