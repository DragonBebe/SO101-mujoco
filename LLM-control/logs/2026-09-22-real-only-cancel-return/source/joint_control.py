...
def bounded_goal(delta, actual, limits):
    ...
    for name, degrees in delta.items():
        if name not in robot.ARM_JOINTS or type(degrees) not in (float,int) or not math.isfinite(degrees) or abs(degrees) > 10:
            raise ValueError('only named arm joints, finite +/-10 degrees per step')
